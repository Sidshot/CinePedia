...
function AnimeGenreRow({ title, genreId, series }) {
        ...
        <section className="mb-10 group/row">
            ...
            <div className="flex items-center justify-between mb-4 px-4">
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                    <span className="w-1 h-6 rounded-full bg-red-600"></span>
                    {title}
                </h2>
                {genreId && (
                    <Link
                        href={`/anime/genre/${genreId}`}
                        className="text-sm font-medium text-red-500 hover:text-red-400 transition-colors bg-red-600/10 hover:bg-red-600/20 px-4 py-2 rounded-full border border-red-600/30"
                    >
                        View All
                    </Link>
                )}
            ...
            <div className="relative">
                ...
                >
                    {series.map((s, index) => (
                        <AnimeCard key={s.id} series={s} index={index} />
                    ))}
                </div>
...
export default memo(AnimeGenreRow);

// Anime Card Component - Premium glassmorphism version
function AnimeCard({ series, index }) {
    ...
    return (
        ...
        >
            ...
            {/* Remove Stream pill as requested */}
            ...
            <div className="relative">
                ...
                <Link href={`/anime/${series.id}`} className="block relative aspect-[2/3] rounded-2xl overflow-hidden bg-gradient-to-br from-gray-900 to-black"
                    style={{
                        clipPath: 'polygon(8px 0%, calc(100% - 8px) 0%, 100% 8px, 100% calc(100% - 8px), calc(100% - 8px) 100%, 8px 100%, 0% calc(100% - 8px), 0% 8px)',
            ...
            <div className="mt-2 px-1">
                <Link href={`/anime/${series.id}`} className="block">
                    <h3 className="font-bold text-[var(--fg)] text-sm truncate hover:text-red-500 transition-colors">
                        {series.name}
                    </h3>
                </Link>
